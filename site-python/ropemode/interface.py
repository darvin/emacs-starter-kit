...
class RopeMode(object):
    ...
    @decorators.local_command() 
    def eldoc_function(self): 
        self._check_project() 
        offset = self.env.get_offset() 
        text = self._get_text() 
        try: 
            begin  = text.rindex('(', 0, offset) 
            try: 
                end = text.rindex(')', begin, offset) 
            except ValueError: #within the parentheses 
                offset = begin - 1 
                return codeassist.get_calltip(self.project, text, offset) 
        except ValueError: 
            return None 
